...
async fn write_sysfs(path: &str, value: &str) -> bool {
    match fs::write(path, format!("{value}\n")).await {
        Ok(_) => { eprintln!("[thermal_core] wrote '{value}' to {path}"); true }
        Err(e) => { eprintln!("[thermal_core] write {path}: {e}"); false }
    }
}
...
pub async fn init() -> Result<(), String> {
    ...
    eprintln!("[thermal_core] init: {core_count} cores, {zone_count} writable zones");
    Ok(())
}
...
pub async fn apply_profile(profile_id: &str) -> Result<(), String> {
    ...
    eprintln!("[thermal_core] applied profile: {profile_id}");
    Ok(())
}
...
pub async fn set_governor(core_id: Option<i32>, governor: &str) -> Result<(), String> {
    ...
    eprintln!("[thermal_core] governor → '{governor}' core={core_id:?}");
    Ok(())
}
...
pub async fn set_max_freq(core_id: Option<i32>, freq_khz: u32) -> Result<(), String> {
    ...
    eprintln!("[thermal_core] max_freq → {freq_khz} kHz core={core_id:?}");
    Ok(())
}
...
pub async fn set_thermal_limit(zone_id: usize, temp_milli: u32) -> Result<(), String> {
    ...
    eprintln!("[thermal_core] zone {zone_id} limit → {temp_milli} mC");
    Ok(())
}
